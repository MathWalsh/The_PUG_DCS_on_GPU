...
void ThreadHandler::ComputeSelfCorrectionInGPU(int32_t u32LoopCount)
{
	...
	index_max_xcorr_subpoint_ptr = (u32LoopCount % 2 == 0) ? index_max_xcorr_subpoint1_ptr : index_max_xcorr_subpoint2_ptr;
	phase_max_xcorr_subpoint_ptr = (u32LoopCount % 2 == 0) ? phase_max_xcorr_subpoint1_ptr : phase_max_xcorr_subpoint2_ptr;
	IGMs_selfcorrection_in_ptr = (u32LoopCount % 2 == 0) ? IGMs_selfcorrection_in1_ptr : IGMs_selfcorrection_in2_ptr;

	if (DcsCfg.save_data_to_file == false) {
		save_data_to_file_local = false;
	}

	if (DcsHStatus.FindFirstIGM[0] == false && DcsHStatus.FirstIGMFound == false && save_data_to_file_local) {

			// Alternate pointers for 1 buffer delay in self-correction
			if (DcsCfg.NIGMs_continuity > 0) {
				if (DcsSelfCorrStatus.self_correction_counter > 0) {
					do_self_correction = true;
				}
				DcsSelfCorrStatus.self_correction_counter += 1;
			}
			else {
				index_max_xcorr_subpoint_ptr = index_max_xcorr_subpoint1_ptr;
				phase_max_xcorr_subpoint_ptr = phase_max_xcorr_subpoint1_ptr;
				IGMs_selfcorrection_in_ptr = IGMs_selfcorrection_in1_ptr;
			}

			if (DcsSelfCorrStatus.UnwrapError_ptr[0] == false && (do_self_correction == true || DcsCfg.NIGMs_continuity == 0)) {
				// Self correction (slow phase correction and resampling)
			

					if (DcsCfg.NIGMs_continuity > 0) {
						cudaMemcpy(phase_max_xcorr_subpoint_ptr + DcsSelfCorrStatus.NIGMs_ptr[0] + DcsCfg.NIGMs_continuity, DcsDStatus.ZPDPhase_buffer_ptr + DcsCfg.NIGMs_continuity, DcsCfg.NIGMs_continuity * sizeof(float), cudaMemcpyDeviceToDevice);
						cudaMemcpy(index_max_xcorr_subpoint_ptr + DcsSelfCorrStatus.NIGMs_ptr[0] + DcsCfg.NIGMs_continuity, DcsDStatus.ZPDMaxIdx_buffer_ptr + DcsCfg.NIGMs_continuity, DcsCfg.NIGMs_continuity * sizeof(double), cudaMemcpyDeviceToDevice);
					...
					cudaStatus = compute_SelfCorrection_GPU(IGMs_selfcorrection_out_ptr, IGMs_selfcorrection_phase_ptr, IGMs_selfcorrection_in_ptr, splineGrid_f0_ptr, splineGrid_dfr_ptr, uniform_grid_ptr,
						...
						phase_max_xcorr_subpoint_ptr, cuda_stream, cudaSuccess, DcsCfg, GpuCfg, &DcsHStatus, DcsDStatus, &DcsSelfCorrStatus);

					cudaStatus = cudaGetLastError();
					if (cudaStatus != cudaSuccess)
					{
						snprintf(errorString, sizeof(errorString), "compute_SelfCorrection_GPU launch failed : %s\n", cudaGetErrorString(cudaStatus));
						ErrorHandler(0, errorString, ERROR_);
					}
					...
					DcsSelfCorrStatus.FirstBlock = (DcsSelfCorrStatus.NBufferAvg == 0) ? true : false;
					DcsSelfCorrStatus.NBufferAvg += 1;
					DcsSelfCorrStatus.NIGMsBlock += DcsSelfCorrStatus.NIGMs_ptr[1];
					if ((DcsSelfCorrStatus.NBufferAvg) % DcsCfg.nb_buffer_average == 0) {
						DcsHStatus.SaveMeanIGM = true;
						DcsHStatus.PlotMeanIGM = true;
						fileCount += 1;
					}
					// Coherent averaging of 1 or multiple buffers
					compute_MeanIGM_GPU(IGM_meanFloatOut_ptr, IGM_meanIntOut_ptr, IGM_mean_ptr, IGMs_selfcorrection_out_ptr, idxSaveIGMs_ptr, cuda_stream, cudaSuccess, DcsCfg, &DcsHStatus, DcsDStatus, &DcsSelfCorrStatus);
					...
					if (DcsHStatus.SaveMeanIGM) {
						DcsSelfCorrStatus.NBufferAvg = 0;
						DcsHStatus.max_xcorr_sum_ptr[0] = 0.0f;
						cudaMemset(DcsDStatus.max_xcorr_sum_ptr, 0, sizeof(double));
						//DcsHStatus.NptsSave = DcsHStatus.segment_size_ptr[2]/2;
						//DcsHStatus.NptsSave = DcsSelfCorrStatus.segment_size_ptr /2;
						DcsHStatus.NptsSave = DcsCfg.ptsPerIGM;
						if (DcsCfg.save_to_float == 1) {

							if (saveCounter == 1) { // for even count (0,2,4...)
							...
							//if (saveCounter == 1) { // for even count (0,2,4...)
							//	cudaMemcpyAsync(IGMsOutFloat1, splineGrid_f0_ptr, DcsHStatus.NptsSave * sizeof(cufftComplex), cudaMemcpyDeviceToHost, cuda_stream);
							//	saveCounter = 0;
							//}
							//else { // for odd count (1,3,5,...)
							//	cudaMemcpyAsync(IGMsOutFloat2, splineGrid_f0_ptr, DcsHStatus.NptsSave * sizeof(cufftComplex), cudaMemcpyDeviceToHost, cuda_stream);
							//	saveCounter = 1;
							//}
						}
						else {
	...
	if (save_data_to_file_local != DcsCfg.save_data_to_file) {

		save_data_to_file_local = (DcsCfg.save_data_to_file == true) ? true : false;
	}
...
void ThreadHandler::ProcessInGPU(int32_t u32LoopCount)
{
	...
	if (DcsHStatus.FindFirstIGM[0] == true && DcsHStatus.FirstIGMFound == false && u32LoopCount > 0) {
		...
		DcsSelfCorrStatus.self_correction_counter = 0;
		do_self_correction = false;
	}
	...
	if (DcsHStatus.FindFirstIGM[0] == false && DcsHStatus.FirstIGMFound == false) {
		...
		cudaMemcpy(DcsHStatus.NIGMs_ptr, DcsDStatus.NIGMs_ptr, 2*sizeof(int), cudaMemcpyDeviceToHost);
		...
		//cudaMemcpy(DcsHStatus.NIGMs_ptr + 1, DcsDStatus.NIGMs_ptr + 1, sizeof(int), cudaMemcpyDeviceToHost);
		...
		if (DcsHStatus.NptsLastIGMBuffer_ptr[1] > 0) {
			cudaMemcpyAsync(LastIGMBuffer_ptr, IGMs_corrected_ptr + (DcsHStatus.segment_size_ptr[0] - DcsHStatus.NptsLastIGMBuffer_ptr[1]),
		...
		if (DcsCfg.save_data_to_file)
		{
			...
			DcsHStatus.NIGMsBlock += DcsHStatus.NIGMs_ptr[1];
		
		}
		else { // We are not saving the IGMs, so we reset variables
			...
			DcsSelfCorrStatus.NIGMsBlock = 0;
			DcsSelfCorrStatus.self_correction_counter = 0;
		}


		if (DcsCfg.NIGMs_continuity > 0) {
			cudaMemcpyAsync(idxSaveIGMs_ptr, DcsDStatus.idxSaveIGMs_ptr, DcsHStatus.NIGMs_ptr[1] * sizeof(int), cudaMemcpyDeviceToDevice, cuda_stream);
			DcsSelfCorrStatus.segment_size_ptr = DcsHStatus.segment_size_ptr[2];
			DcsSelfCorrStatus.NIGMs_ptr[0] = DcsHStatus.NIGMs_ptr[0];
			DcsSelfCorrStatus.NIGMs_ptr[1] = DcsHStatus.NIGMs_ptr[1];
			DcsSelfCorrStatus.UnwrapError_ptr[0] = DcsHStatus.UnwrapError_ptr[0];
	...
	if (u32LoopCount == 0) {
		...
		LogStats(fileHandle_log_DCS_Stats, fileCount, u32LoopCount, DcsHStatus.NIGMs_ptr[0], DcsHStatus.NIGMs_ptr[1], DcsHStatus.NIGMsTot,
			...
			static_cast<float>(DcsCfg.sampling_rate_Hz / DcsHStatus.previousptsPerIGM_ptr[0]),
	...
	if (u32LoopCount > 1) {
		...
		LogStats(fileHandle_log_DCS_Stats, fileCount, u32LoopCount - 1, DcsHStatus.NIGMs_ptr[0], DcsHStatus.NIGMs_ptr[1], DcsHStatus.NIGMsTot,
			DcsHStatus.NIGMsAvgTot, 100 * static_cast<float>(DcsHStatus.NIGMsAvgTot) / static_cast<float>(DcsHStatus.NIGMsTot),
			DcsHStatus.UnwrapError_ptr[0], DcsHStatus.NotEnoughIGMs[0], static_cast<int>(delay),
			static_cast<double>(DcsCfg.sampling_rate_Hz) / DcsHStatus.previousptsPerIGM_ptr[0], 
			DcsCfg.measurement_name, DcsCfg.save_data_to_file);
	}
	...
	if (DcsHStatus.FindFirstIGM[0] && DcsHStatus.FirstIGMFound) { // This is because we did not do the self-correction for the first batch
		...
		DcsHStatus.NIGMs_ptr[0] = static_cast<int>(std::round((DcsHStatus.segment_size_ptr[0]
		...
		if (DcsHStatus.LastIdxLastIGM < DcsHStatus.segment_size_ptr[0]) { // What happens if LastIdxLastIGM =segment_size_ptr (TO DO)

			// The remaining points go in the buffer
			DcsHStatus.NptsLastIGMBuffer_ptr[0] = DcsHStatus.segment_size_ptr[0] - (DcsHStatus.LastIdxLastIGM + 1);

			// We copy the data to the buffer  for the next segment
			cudaMemcpyAsync(LastIGMBuffer_ptr, IGMs_corrected_ptr + (DcsHStatus.segment_size_ptr[0] - DcsHStatus.NptsLastIGMBuffer_ptr[0]),
			...
			DcsHStatus.NptsLastIGMBuffer_ptr[0] = DcsHStatus.segment_size_ptr[0] - DcsHStatus.LastIdxLastIGM;
			cudaMemcpyAsync(LastIGMBuffer_ptr, IGMs_corrected_ptr + (DcsHStatus.segment_size_ptr[0] - DcsHStatus.NptsLastIGMBuffer_ptr[0]),
				DcsHStatus.NptsLastIGMBuffer_ptr[0] * sizeof(cufftComplex), cudaMemcpyDeviceToDevice, cuda_stream); // can it be done on stream1 ??  transfer data in buffer for next segment
	...
	if (DcsHStatus.FindFirstIGM[0] == false) // For buffers where we do the self-correction
	{
		...
		DcsHStatus.segment_size_ptr[1] = DcsHStatus.segment_size_ptr[0] + DcsHStatus.NptsLastIGMBuffer_ptr[0];
	...
	if (DcsCfg.nb_phase_references == 0) {
		...
		else if (DcsCfg.spectro_mode == 2)
		{
			//Create the dfr reference 
			//Here we assume that the signals are placed in this oder: IGMs, foptCW1_C1, foptCW1_C2, foptCW2_C1, foptCW2_C2;
			cudaStatus = compute_dfr_high_harmonic_angle_GPU(optical_ref_dfr_angle_ptr, optical_ref1_angle_ptr, unwrapped_dfr_phase_ptr, filtered_signals_ptr + 1 * segment_size_per_channel,
				filtered_signals_ptr + 2 * segment_size_per_channel, filtered_signals_ptr + 3 * segment_size_per_channel, filtered_signals_ptr + 4 * segment_size_per_channel,
				ref1_offset_buffer_in_ptr, ref1_offset_buffer_out_ptr, ref2_offset_buffer_in_ptr, ref2_offset_buffer_out_ptr, segment_size_per_channel, cuda_stream,
				cudaSuccess, DcsCfg, GpuCfg);

			if (cudaStatus != cudaSuccess) {
				snprintf(errorString, sizeof(errorString), "compute_dfr_high_harmonic_angle_GPU launch failed: %s", cudaGetErrorString(cudaStatus));
				ErrorHandler(0, errorString, ERROR_);
			}
		...
		else if (DcsCfg.spectro_mode == 3)
		{
			//Create the dfr reference 
			//Here we assume that the signals are placed in this oder: IGMs, foptCW1_C1, foptCW1_C2, foptCW2_C1, foptCW2_C2;
			cudaStatus = compute_ref_angle_GPU(optical_ref1_angle_ptr, filtered_signals_ptr + 1 * segment_size_per_channel,
				filtered_signals_ptr + 2 * segment_size_per_channel, ref1_offset_buffer_in_ptr, ref1_offset_buffer_out_ptr, segment_size_per_channel, cuda_stream, cudaSuccess, DcsCfg, GpuCfg);

			if (cudaStatus != cudaSuccess) {
				snprintf(errorString, sizeof(errorString), "compute_ref_angle_GPU launch failed: %s", cudaGetErrorString(cudaStatus));
				ErrorHandler(0, errorString, ERROR_);
			}

			DcsHStatus.Unwrapdfr = false;
			// Unwrap the phase of the dfr signal
			cudaStatus = unwrap_phase_GPU(unwrapped_dfr_phase_ptr, optical_ref1_angle_ptr, two_pi_count_ptr, blocks_edges_cumsum_ptr, increment_blocks_edges_ptr, segment_size_per_channel,
				cuda_stream, cudaSuccess, DcsCfg, GpuCfg, &DcsHStatus, DcsDStatus); // (fast unwrap with 128 threads on 4090 GPU)
		...
		else if (DcsCfg.spectro_mode == 3 && DcsCfg.do_fast_resampling == 1) {
	...
	if ((DcsCfg.do_phase_projection == 0 && (DcsCfg.spectro_mode == 0 || DcsCfg.spectro_mode == 2)) || DcsCfg.nb_phase_references == 0 || DcsCfg.nb_phase_references == 1) {
		...
		rotate_IGMs_phase_GPU(IGMs_corrected_ptr, DcsHStatus.IGMs_rotation_angle, DcsCfg.slope_self_correction, DcsHStatus.segment_size_ptr[0],	blocksRotate, 128, cuda_stream, cudaSuccess);
		if (cudaStatus != cudaSuccess)
		{
		...
		DcsHStatus.IGMs_rotation_angle -= static_cast<double>(DcsHStatus.segment_size_ptr[0]) * DcsCfg.slope_self_correction;
		DcsHStatus.IGMs_rotation_angle = atan2(sin(DcsHStatus.IGMs_rotation_angle), cos(DcsHStatus.IGMs_rotation_angle));
	...
	if (DcsHStatus.NptsLastIGMBuffer_ptr[0] > 0 && DcsHStatus.FindFirstIGM[0] == false) {
		...
		cudaMemcpyAsync(IGMs_selfcorrection_in_ptr + DcsHStatus.NptsLastIGMBuffer_ptr[0], IGMs_corrected_ptr,
			DcsHStatus.segment_size_ptr[0] * sizeof(cufftComplex), cudaMemcpyDeviceToDevice, cuda_stream);
		...
		cudaMemcpyAsync(IGMs_selfcorrection_in_ptr, IGMs_corrected_ptr, DcsHStatus.segment_size_ptr[0] * sizeof(cufftComplex), cudaMemcpyDeviceToDevice, cuda_stream);
	}
...
void ThreadHandler::PopulateHostStatus(DCSHostStatus &DcsHStatus) {
	// For DCSHostStatus
...
void ThreadHandler::PopulateDeviceStatus(DCSDeviceStatus& DcsDStatus) {

	// For DCSDeviceStatus
		// 2 ref resampling
	double* zero_val = new double[7];
	zero_val[0] = 0.0f;
	zero_val[1] = 0.0f;
	zero_val[2] = 0.0f;
	zero_val[3] = 0.0f;
	...
	cudaMemcpy(DcsDStatus.start_slope_ptr, zero_val, 3 * sizeof(double), cudaMemcpyHostToDevice);
	...
	cudaMemcpy(DcsDStatus.end_slope_ptr, zero_val, 3 * sizeof(double), cudaMemcpyHostToDevice);


	cudaStatus = cudaMalloc((void**)&DcsDStatus.last_projected_angle_ptr, 5 * sizeof(double));
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for last_projected_angle_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}

	cudaMemcpy(DcsDStatus.last_projected_angle_ptr, zero_val, 5 * sizeof(double), cudaMemcpyHostToDevice);
	// find_IGMs_ZPD_GPU
	...
	cudaMemset(DcsDStatus.UnwrapError_ptr, false, sizeof(bool));
	...
	cudaStatus = cudaMalloc((void**)&DcsDStatus.xcorr_data_out_GUI_ptr, 30 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(double));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for IGM_weights: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}
	//cudaMemset(DcsDStatus.xcorr_data_out_GUI_ptr, 0, 30 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(double));
	...
	if (DcsCfg.NIGMs_continuity > 0) {
		...
		cudaStatus = cudaMalloc((void**)&DcsDStatus.ZPDPhase_buffer_ptr, (2*DcsCfg.NIGMs_continuity + 2) * sizeof(float));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
		if (cudaStatus != cudaSuccess) {
			snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for ZPDPhase_buffer_ptr: %s\n", cudaGetErrorString(cudaStatus));
			ErrorHandler(cudaStatus, errorString, ERROR_);
		}

		cudaMemcpy(DcsDStatus.ZPDPhase_buffer_ptr, zero_val_f, (2 * DcsCfg.NIGMs_continuity + 2) * sizeof(float), cudaMemcpyHostToDevice);

		cudaStatus = cudaMalloc((void**)&DcsDStatus.ZPDMaxIdx_buffer_ptr, (2 * DcsCfg.NIGMs_continuity + 1) * sizeof(double));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
		if (cudaStatus != cudaSuccess) {
			snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for ZPDMaxIdx_buffer_ptr: %s\n", cudaGetErrorString(cudaStatus));
			ErrorHandler(cudaStatus, errorString, ERROR_);
		}


		cudaMemcpy(DcsDStatus.ZPDMaxIdx_buffer_ptr, zero_val, (2 * DcsCfg.NIGMs_continuity + 1) * sizeof(double), cudaMemcpyHostToDevice);
...
void ThreadHandler::PopulateSelfCorrectionStatus(DCSSelfCorrectionStatus& DcsSelfCorrStatus, DCSCONFIG& DcsCfg) {
	
	char errorString[255]; // Buffer for the error message
	cudaError_t  cudaStatus = (cudaError_t)0;

	DcsCfg.NIGMs_continuity = NIGMs_continuity;
	DcsProcessingPtr->modify_DCSCONFIG_field("NIGMs_continuity", &NIGMs_continuity);
...
void ThreadHandler::AllocateGPUBuffers()
{
	...
	PopulateHostStatus(DcsHStatus);

	PopulateSelfCorrectionStatus(DcsSelfCorrStatus, DcsCfg);

	PopulateDeviceStatus(DcsDStatus);
	...
	cudaStatus = cudaMalloc(&increment_blocks_edges_ptr, segment_size_per_channel * sizeof(int));
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for blocks_edges_cumsum_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}
	cudaMemset(increment_blocks_edges_ptr, 0, segment_size_per_channel * sizeof(int));
	...
	cudaStatus = cudaMalloc((void**)&IGMs_selfcorrection_in1_ptr, (segment_size_per_channel + 2 * DcsCfg.ptsPerIGM) * sizeof(cufftComplex));
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for IGMs_selfcorrection_in1_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}

	cudaStatus = cudaMalloc((void**)&IGMs_selfcorrection_in2_ptr, (segment_size_per_channel + 2 * DcsCfg.ptsPerIGM) * sizeof(cufftComplex));
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for IGMs_selfcorrection_in2_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}
	...
	cudaStatus = cudaMalloc((void**)&index_mid_segments_ptr, 10 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(int));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for index_mid_segments_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}

	cudaStatus = cudaMalloc((void**)&unwrapped_selfcorrection_phase_ptr, 10 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(double));
	if (cudaStatus != cudaSuccess) {
	...
	cudaStatus = cudaMalloc((void**)&index_max_xcorr_subpoint1_ptr, 10 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(double));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for index_max_xcorr_subpoint1_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);

	}

	cudaStatus = cudaMalloc((void**)&index_max_xcorr_subpoint2_ptr, 10 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(double));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for index_max_xcorr_subpoint2_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}

	cudaStatus = cudaMalloc((void**)&phase_max_xcorr_subpoint1_ptr, 10 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(float));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for phase_max_xcorr_subpoint1_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}

	cudaStatus = cudaMalloc((void**)&phase_max_xcorr_subpoint2_ptr, 10 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(float));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for phase_max_xcorr_subpoint2_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}
	...
	cudaStatus = cudaMalloc((void**)&idxSaveIGMs_ptr, 3 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(int));  // Factor of 3 to make sure we always have enough space depending on the variations in dfr 
	if (cudaStatus != cudaSuccess) {
		snprintf(errorString, sizeof(errorString), "Failed to allocate GPU memory for idxSaveIGMs_ptr: %s\n", cudaGetErrorString(cudaStatus));
		ErrorHandler(cudaStatus, errorString, ERROR_);
	}
	cudaMemset(idxSaveIGMs_ptr, 0, 3 * (segment_size_per_channel / DcsCfg.ptsPerIGM) * sizeof(int));
			...
			if (u32Elapsed >= DcsCfg.real_time_display_refresh_rate_ms) {
				...
				if (lock.owns_lock()) {
					...
					case interferogram_filtered: {
						int idxStart = static_cast<int>(std::round(3 * DcsCfg.ptsPerIGM / 2 - (threadControlPtr->displaySignal1_size) / 16) - DcsHStatus.NptsLastIGMBuffer_ptr[0]); // ptsPerIGM/2 - NptsSignal/2
					...
					case interferogram_filtered: {
						...
						int idxStart = static_cast<int>(std::round(3 * DcsCfg.ptsPerIGM / 2 - (threadControlPtr->displaySignal2_size) / 16) - DcsHStatus.NptsLastIGMBuffer_ptr[0]); // ptsPerIGM/2 - NptsSignal/2
						if (idxStart < 0)
							idxStart = 0;
					...
					case xcorr_data: {
						...
						float* tempUnwrapValue = new float[1];
						//tempUnwrapValue[0] = 0.0f;
						if (DcsHStatus.UnwrapError_ptr[0] == true) {
							tempUnwrapValue[0] = 1.0f;
						}

						memcpy(threadControlPtr->displaySignalXcorr_ptr + 4 * DcsHStatus.NIGMs_ptr[0], tempUnwrapValue, sizeof(float));
						cudaMemcpy(threadControlPtr->displaySignalXcorr_ptr, DcsDStatus.xcorr_data_out_GUI_ptr, 4 * DcsHStatus.NIGMs_ptr[0] * sizeof(float), cudaMemcpyDeviceToHost);
						threadControlPtr->displaySignalXcorr_size = (4 * DcsHStatus.NIGMs_ptr[0] + 1) * sizeof(float); // We change size of buffer here because the nb of igms will change
...
void ThreadHandler::UpdateProgress(int32_t u32LoopCount)
{
	...
	if (u32Elapsed > 0)
	{
		if ((u32LoopCount > 3 && u32Elapsed / 1000 / 1 > console_status_update_counter && processing_choice == RealTimeAcquisition_Processing) || (u32LoopCount > 3 && processing_choice == ProcessingFromDisk || processing_choice == RealTimeAcquisition))
		{
		...
		dTotal = 1.0 * CardTotalData * DcsCfg.nb_bytes_per_sample / 1000000.0;		// Mega bytes
			...
			printf("Batch number: %d, Dfr: %.5f Hz, AvgBatch: %.1f%%%, AvgTot: %.1f%%%-5s", fileCount, static_cast<double>(DcsCfg.sampling_rate_Hz) / DcsHStatus.previousptsPerIGM_ptr[0], DcsHStatus.percentageAvgBlock,
			...
			console_status_update_counter += 1;
			...
			if (u32Elapsed/1000 / DcsCfg.console_status_update_refresh_rate_s > console_update_counter) {
				...
				snprintf(message, sizeof(message), "\nStatus Update: LoopCount: %d, Total: %0.2f MB, Rate: %6.2f MB/s, Elapsed time: %u:%02u:%02u\nBatch number: %d, Dfr: %.5f Hz, AvgBatch: %.1f%%%, AvgTot: %.1f%%%-5s", u32LoopCount, dTotal, dRate, 
					(unsigned int)h, (unsigned int)m, (unsigned int)s, fileCount, static_cast<double>(DcsCfg.sampling_rate_Hz) / DcsHStatus.previousptsPerIGM_ptr[0], DcsHStatus.percentageAvgBlock,
					100 * static_cast<double>(DcsHStatus.NIGMsAvgTot) / static_cast<double>(DcsHStatus.NIGMsTot), "");
