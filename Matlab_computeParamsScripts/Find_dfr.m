...
% Use stronget IGM to find template
[~,idxmid] = max(abs(IGMs(ptsPerIGM/2:end-ptsPerIGM/2)));
idxmid = idxmid + ptsPerIGM/2 - 1;

if winHW ~= -1 % User input template size

    % Re-centering in the middle of the 2 sides JGE 12/02/2019
    idx1 = find(abs(IGMs(idxmid-ptsPerIGM/2:idxmid+ptsPerIGM/2))/max(abs(IGMs(idxmid-ptsPerIGM/2:idxmid+ptsPerIGM/2)))>0.3, 1, 'first'); % Changed the boundary for asymmetric IGMs MW 17/01/2024
    idx2 = find(abs(IGMs(idxmid-ptsPerIGM/2:idxmid+ptsPerIGM/2))/max(abs(IGMs(idxmid-ptsPerIGM/2:idxmid+ptsPerIGM/2)))>0.3, 1, 'last');

    if (idx2 - ptsPerIGM/2) > winHW || (ptsPerIGM/2 - idx1) > winHW % The points found are too far, lets try with a higher threshold

        idx1 = find(abs(IGMs(idxmid-ptsPerIGM/2:idxmid+ptsPerIGM/2))>0.5, 1, 'first'); % Changed the boundary for asymmetric IGMs MW 17/01/2024
        idx2 = find(abs(IGMs(idxmid-ptsPerIGM/2:idxmid+ptsPerIGM/2))>0.5, 1, 'last');
...
IGMs_MF = fftfilt(double(MF),double(IGMs));
...
% Finding peaks and keeping only consecutive peaks based on dfr
[~, locs] = findpeaks(abs(IGMs_MF(ptsPerIGM/2:end-ptsPerIGM/2)),'MinPeakDistance',(3/4)*ptsPerIGM, 'MinPeakHeight', threshold);
locs  = locs + ptsPerIGM/2-1;
...
slope(2) = [];
slope = round(slope/(2*pi/mean(diff(locs))))*(2*pi/mean(diff(locs)));
IGMs = IGMs.*exp(-1j*slope*(0:N-1));

% Find subpoint locs_positions
pattern = IGMs(idxmid-winHW:idxmid+winHW);
[true_locs, ~] = estimatePhase(IGMs,pattern,locs,winHW,n);

% Update parameters
dfr = fs/mean(diff(true_locs));
ptsPerIGM = round(fs/dfr/2)*2;
idxStart = locs(1) - round(1.01*ptsPerIGM/2); % Crop data to keep only full IGMs
idxEnd = locs(end) + round(1.01*ptsPerIGM/2);
